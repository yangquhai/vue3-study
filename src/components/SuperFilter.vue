<template>
 ...
 const selectList = computed(()=>{
        let selectList = []
         for(let i = 0;i<this.tabListData.searchCondition.length;i++){
           // console.log(this.tabListData.searchCondition[i])
           if(this.tabListData.searchCondition[i].type=="select")
              {
                 selectList.push(this.tabListData.searchCondition[i])
                 for(let j=0;j<this.tabListData.searchCondition[i].condition.length;j++){
                 if(this.tabListData.searchCondition[i].condition[j].select)
                 // console.log(i)
                 this.value.push({luoji:this.tabListData.searchCondition[i].condition[j].luoji})
 ...
 const textList = computed(()=>{
  let textList = []
         for(let i=0;i<this.tabListData.searchCondition.length;i++){
           // console.log(this.tabListData.searchCondition[i])
           if(this.tabListData.searchCondition[i].type=="text")
           {
                 textList.push(this.tabListData.searchCondition[i])
                 // this.value1.push({luoji:'包含'})
                 // console.log(this.tabListData.searchCondition[i])
                 for(let j=0;j<this.tabListData.searchCondition[i].condition.length;j++){
                 if(this.tabListData.searchCondition[i].condition[j].select)
                 // console.log(i)
                 this.value1.push({luoji:this.tabListData.searchCondition[i].condition[j].luoji})
                 // this.value1[valueIndex].luoji = this.tabListData.searchCondition[i].condition[j].luoji
                 }
